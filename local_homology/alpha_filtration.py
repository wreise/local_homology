...
## Coning Variant
def compute_local_homology_alpha(point_cloud, x0, epsilon, max_dimension):
...
def build_local_complex_alpha(point_cloud, point, epsilon, max_dimension):
    ...
    point_cloud_local = point_cloud[is_in_ball]
    point_cloud_dist_local = point_cloud_dist[is_in_ball][:, is_in_ball]
    local_boundary = distance_to_boundary[is_in_ball]

    point_cloud_local_stared = np.concatenate([point_cloud_dist_local/2., local_boundary.T],
                                              axis=0)
